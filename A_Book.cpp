...
void dfs(int n){
    ...
    val[n]=0;
    for(auto x:graph[n]){
        if(st.find(x)!=st.end()){cycle=true;}
        if(!vis[x]) dfs(x);
        val[n]=max(val[x],val[n]);
        if(x>n)val[n]=max(val[n],val[x]+1);
        ans=max(ans,val[n]);
    }
    if(graph[n].size()==0)val[n]=1;
}

void reincarnate(){
    graph.clear();val.clear();vis.clear();ans=0;
    cycle=false;
}
