...
void solve(){
    ...
    rep(i,n){
        int t=v[i];
        if(*st.upper_bound(t)!=INF && taken==true && (*st.upper_bound(t)>temp)){
            amount=tempp;
            amount*=t;
            anstobe[ind]=0;
            temp=*st.lower_bound(t);
            deb(temp);
            anstobe[i]=1;
            taken=true;
        }

        if(*st.upper_bound(t)!=INF && taken==false){
            ind=i;
            tempp=amount;
            amount*=t;
            temp=*st.lower_bound(t);
