...
void solve(){
    int n,m,xxx;cin>>n>>m>>xxx;
    char arr[n][m];
    vii S;
    vector<vector<pair<int,int>>>mp(26);
    S.push_back({10000,100000});
    set<char>st;
    ...
    rep(i,n){
        rep(j,m){
            cin>>arr[i][j];
            if(arr[i][j]!='S'){
                mp[(arr[i][j]-32-((int)'A'))].push_back({i,j});
                st.insert(arr[i][j]);
            }
            else{
                S.push_back({i,j});
    ...
    string s;cin>>s;
    for(auto x:s){
        if(st.find(x)==st.end() && st.find(x+32)==st.end()){
            cout<<-1;
            return;
        }
        // deb(x);
        ...
        if((int)x<95){
            for(auto xx:S){
                for(auto it:mp[x-(int)'A']){
                    double d=sqrt((xx.first-it.first)*(xx.first-it.first)+(xx.second-it.second)*(xx.second-it.second));
                    if(d<dis){
                        dis=d;
                    }
                }
