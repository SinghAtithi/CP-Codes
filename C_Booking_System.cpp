...
void solve(){
    ...
    vii ans;
    int q;cin>>q;
    vi table(q);vin(x,table);
    multiset<int>st;
    for(auto x:table)st.insert(x);
    // deb(st);
    int count, sum;
    count=sum=0;
    int i=0;
    for(auto x:v){
        ...
        int ind=-1, temp=INF;
        rep(i,table.size()){
            // loop and find the table which will suit the best
            if(table[i]>=x[0] && table[i]<temp && st.find(table[i])!=st.end())temp=table[i],ind=i;
        }
        if(ind!=-1){
            ans.pb({x[2],ind+1});
            st.erase(temp);
