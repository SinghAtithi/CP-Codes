...
void solve(){
    ...
    int ans=0;
    loop(i,1,n){
        ans+=max(int(0), v[i].first-v[i-1].second);
    }
    ans+=max(int(0),v[0].first-v[n-1].second);
    int temp=v[0].first-max(int(0),v[0].first-v[n-1].second);
    loop(i,1,n){
        temp=min(temp,v[i].first-max(int(0),v[i].first-v[i-1].second));
    }
        ans+=temp;
