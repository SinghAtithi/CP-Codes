...
/*-----------------------------------D-E-B-U-G-----------------------------------------------*/
bool check(int ind, vi &v, int target, int k){
    if(target==2 && k>=(v.size()+1)/2)return true;
    if(ind==v.size())return false;
    return (check(ind+1,v,target,k) ||
    check(ind+1,v,__gcd(target,v[ind]),k+1));
}
